...
def populate():
    ...
    mak, _ = Institutions.objects.get_or_create(
        name="Makerere University", defaults={"email_domain": "mak.ac.ug"}
    )
    ...
    for college_name, departments in colleges_data.items():
        college, _ = Colleges.objects.get_or_create(institution=mak, name=college_name)
        for dept_name in departments:
            department, _ = Departments.objects.get_or_create(college=college, name=dept_name)
